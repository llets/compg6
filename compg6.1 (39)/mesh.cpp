...
void ai4ToGlm(glm::mat4& m, const aiMatrix4x4& AssimpMatrix)
{
...
void ai3ToGlm(glm::mat4& m, const aiMatrix3x3& AssimpMatrix)
{
...
glm::mat4 InitScaleTransf(float ScaleX, float ScaleY, float ScaleZ)
{
    glm::mat4 m;
...
bool Mesh::LoadMesh(const string& Filename, bool WithAdjacencies)
{
    ...
    if (m_pScene) {  
        ai4ToGlm(m_GlobalInverseTransform, m_pScene->mRootNode->mTransformation);
...
{  
    ...
    // Counting the number of vertices and indices
    ...
    // Reserving space in the vectors for the vertex attributes and indices
    ...
    // Initialization of the meshes in the scene one by one
    for (unsigned int i = 0 ; i < m_Entries.size() ; i++) {
    ...
    // Generation and population the buffers with vertex attributes and the indices
...
void Mesh::LoadBones(unsigned int MeshIndex, const aiMesh* pMesh, vector<VertexBoneData>& Bones)
{
    for (unsigned int i = 0 ; i < pMesh->mNumBones ; i++) {                
        ...
        if (m_BoneMapping.find(BoneName) == m_BoneMapping.end()) {
            ...
            ai4ToGlm(m_BoneInfo[BoneIndex].BoneOffset, pMesh->mBones[i]->mOffsetMatrix);
...
{    
    ...
    ai4ToGlm(NodeTransformation, pNode->mTransformation);
    ...
    if (pNodeAnim) {
        ...
        glm::mat4 ScalingM;
        ScalingM = InitScaleTransf(Scaling.x, Scaling.y, Scaling.z);
        
        // Interpolate rotation and generate rotation transformation matrix
        aiQuaternion RotationQ;
        CalcInterpolatedRotation(RotationQ, AnimationTime, pNodeAnim);    
        glm::mat4 RotationM;
        ai3ToGlm(RotationM, RotationQ.GetMatrix());

        // Interpolate translation and generate translation transformation matrix
        aiVector3D Translation;
        CalcInterpolatedPosition(Translation, AnimationTime, pNodeAnim);
        glm::mat4 TranslationM;
        TranslationM = InitTranslationTransf(Translation.x, Translation.y, Translation.z);
